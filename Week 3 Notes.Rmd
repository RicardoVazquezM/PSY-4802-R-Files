...
After Today's Lecture, you should be able to do the following: - Evaluate mathematical expressions and logical statements in R - Describe how to use common R functions - Explain the differences between variables, vectors, and dataframes - Explain how to obtain a subset of your data - Describe your data effectively
...
A similar process can be used to determine if something is greater than, less than, greater than or equal to, and less than or equal to. **Aside:** I use these a lot to clean developmental data
...
And statements use `&` OR `&&` (if you have more things to evaluate) USE DOUBLE &&
...
```{r}
animals <- c("dog", "cat", "penguin", "turtle","snake", "jaguar")
```

2)Make a vector of the number of cookies you think each animal could eat

```{r}
animalCookies <- c(2,1,1,1,2,5)
```
...
```{r}
tvShows <- c("Breaking Bad", "The Walking Dead", "Game of Thrones", "Survivor")
ratings <- c(4, 3.5, 3.5, 4.5)
```

3.  Make these vectors into a data frame.

```{r}
tvShowRatings <- cbind.data.frame(tvShows, ratings)
```
...
**Indexing** We'll started with indexing because it is similar to what we did above. You can write dataframeName[logical statement] to get a portion of the data.

For instance, we could use indexing to get only cars in the mtcars dataset with 4 gears.

```{r}
mtcars[mtcars$gear==4,]
fourGear <-subset(mtcars, mtcars$gear==4)
fourGear
```
